        ...
        @if (Auth::guard('organisasi')->check())
            ...
            >
                <button
                    ...
                    class="w-full flex items-center justify-between px-4 py-3 rounded-lg font-medium transition-colors focus:outline-none
                               {{ request()->routeIs('organisasi.rekrutmen.index') ? 'bg-blue-50 text-blue-700' : 'text-gray-600 hover:bg-gray-100 hover:text-gray-900' }}"
                >
                    <div
                        class="flex items-center"
                        @click.stop="window.location.href='{{ route('organisasi.rekrutmen.index') }}'"
                    >
                        <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
